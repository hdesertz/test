...
class CreateSecurityActionTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->increments('id');
			$table->tinyInteger('address')->comment('(1是前台，2是后台)');
			$table->integer('user_id')->comment('用户id');
			$table->integer('last_ip')->nullable()->comment('上次登录ip');
			$table->integer('ip')->comment('当前ip');
			$table->tinyInteger('login_count')->comment('登录次数');
			$table->tinyInteger('pass_is_true')->comment('(1正确2错误)');
			...
			$table->string('refer')->default('')->comment('refer');
